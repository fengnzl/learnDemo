var printRecords = recordIds => {
	recordIds.map(studentId => studentRecords.find(record => record.id == studentId))
...
var remindUnpaid = recordIds => studentRecords.filter(record => !record.paid && recordIds.includes(record.id))
